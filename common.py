...
#]
LR_SCHEDULE = [    # (multiplier, epoch to start) tuples
    (1.0, 5), (0.1, 30), (0.01, 60), (0.001, 90), (0.0001, 120)
]
...
class PiecewiseConstantDecayWithWarmup(
  ...
  def __init__(self, batch_size, epoch_size, warmup_epochs, boundaries,
    ...
    self.step_boundaries = [float(steps_per_epoch) * float(x) for x in boundaries]
    ...
    with tf.compat.v1.name_scope(self.name, 'PiecewiseConstantDecayWithWarmup',
      ...
      def piecewise_lr(step):
        return tf.compat.v1.train.piecewise_constant(
            tf.cast(step, tf.float32), self.step_boundaries, self.lr_values)
