...
from regularization import mixup
...
def run(flags_obj):
  ...
  with distribution_utils.get_strategy_scope(strategy):
    ...
    if flags_obj.use_dropblock:
      logging.info(float(flags_obj.dropblock_kp[0]))
      logging.info(float(flags_obj.dropblock_kp[1]))
      keep_prob_fn = dropblock.keep_prob_decay(starter_kp=float(flags_obj.dropblock_kp[0]),
                                               end_kp=float(flags_obj.dropblock_kp[1]),
                                               num_images=imagenet_preprocessing.NUM_IMAGES['train'],
                                               batch_size=flags_obj.batch_size,
                                               train_epochs=flags_obj.train_epochs)
      drbl_kp_callback = dropblock.DropblockKeepProbScheduler(keep_prob_fn, model)
    else:
      keep_prob_fn = None
      drbl_kp_callback = None

    if flags_obj.learning_rate_decay_type == 'piecewise':
        lr_schedule = common.PiecewiseConstantDecayWithWarmup(
            batch_size=flags_obj.batch_size,
            epoch_size=imagenet_preprocessing.NUM_IMAGES['train'],
            warmup_epochs=flags_obj.lr_warmup_epochs,
            boundaries=list(flags_obj.piecewise_lr_boundary_epochs),
            multipliers=list(flags_obj.piecewise_lr_decay_rates),
    ...
    manager = tf.train.CheckpointManager(
      checkpoint, directory=flags_obj.model_dir, max_to_keep=10
    )
    latest_checkpoint = tf.train.latest_checkpoint(flags_obj.model_dir)
    if latest_checkpoint:
      checkpoint.restore(manager.latest_checkpoint)
      current_step = optimizer.iterations.numpy()
      logging.info("Load checkpoint %s, current_step %s", latest_checkpoint, current_step)
    ...
    def step_fn(inputs):
      """Per-Replica StepFn."""
      if flags_obj.kd_temp > 0.0:
        images, labels, teacher_logits = inputs
        teacher_labels = tf.nn.softmax(teacher_logits / float(flags_obj.kd_temp))
      else:
        images, labels = inputs
        teacher_labels = None

      with tf.GradientTape() as tape:
        onehot_labels = tf.one_hot(tf.cast(tf.squeeze(labels), tf.int32), flags_obj.num_classes)
        if flags_obj.mixup_alpha > 0.0:
          logging.info('mixup: {}'.format(flags_obj.mixup_alpha))
          images, onehot_labels, teacher_labels = mixup.mixup(images, onehot_labels,
                                                              y_t=teacher_labels,
                                                              alpha=flags_obj.mixup_alpha)
          logging.info(images)
          logging.info(onehot_labels)

        # if flags_obj.kd_temp > 0.0:
        #   logits, logits_temp = model(images, training=True)
        # else:
        #   logits = model(images, training=True)
        logits = model(images, training=True)
        prediction = tf.nn.softmax(logits)
        prediction_kd = tf.nn.softmax(logits / float(flags_obj.kd_temp))

        if flags_obj.label_smoothing > 0.0:
          logging.info('label smoothing: {}'.format(flags_obj.label_smoothing))
        prediction_loss = tf.keras.losses.categorical_crossentropy(onehot_labels, prediction,
                                                                   label_smoothing=flags_obj.label_smoothing)
        loss = tf.reduce_sum(prediction_loss) * (1.0/ flags_obj.batch_size)
        num_replicas = tf.distribute.get_strategy().num_replicas_in_sync
        logging.info('num_replicas {}'.format(num_replicas))

        if flags_obj.kd_temp > 0.0:
          prediction_loss_kd = tf.keras.losses.categorical_crossentropy(teacher_labels,
                                                                        prediction_kd)
          loss_kd = tf.reduce_sum(prediction_loss_kd) * (1.0 / flags_obj.batch_size)
          loss += flags_obj.kd_temp * flags_obj.kd_temp * loss_kd

        if flags_obj.single_l2_loss_op:
          l2_loss = flags_obj.weight_decay * 2 * tf.add_n([
              tf.nn.l2_loss(v)
    ...
    if flags_obj.eval_only:
      ...
      print('Test loss: %s, accuracy: %s', test_loss.result().numpy(), test_accuracy.result().numpy())
      import sys
      sys.exit(0)
    ...
    for epoch in range(current_step // per_epoch_steps, train_epochs):
      ...
      while steps_in_current_epoch < per_epoch_steps:
        ...
        if drbl_kp_callback: drbl_kp_callback.on_batch_begin(steps_in_current_epoch+epoch*per_epoch_steps)
      ...
      if flags_obj.enable_checkpoint_and_export and not flags_obj.eval_only:
        # checkpoint_name = checkpoint.save(
        #     os.path.join(flags_obj.model_dir,
        #                  'model.ckpt-{}'.format(epoch + 1)))
        # save_path = os.path.join(flags_obj.model_dir,'model.ckpt-{}'.format(epoch + 1))
        checkpoint_name = manager.save(checkpoint_number=epoch + 1)
      ...
      if summary_writer:
        ...
        with summary_writer.as_default():
          ...
          if keep_prob_fn:
            kp_for_monitor = keep_prob_fn(current_steps)
            if callable(kp_for_monitor):
              kp_for_monitor = kp_for_monitor()
            tf.summary.scalar('keep_prob', kp_for_monitor, current_steps)
